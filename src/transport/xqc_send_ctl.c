...
static xqc_bool_t
xqc_send_ctl_is_ml_cc(xqc_send_ctl_t *send_ctl)
{
    const xqc_cong_ctrl_callback_t *cb = send_ctl->ctl_cong_callback;

    if (cb == NULL) {
        return XQC_FALSE;
    }

    return cb->xqc_cong_ctl_init == xqc_ml_cc_cb.xqc_cong_ctl_init
        && cb->xqc_cong_ctl_on_ack == xqc_ml_cc_cb.xqc_cong_ctl_on_ack
        && cb->xqc_cong_ctl_get_cwnd == xqc_ml_cc_cb.xqc_cong_ctl_get_cwnd;
}
...
xqc_send_ctl_on_packet_sent(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_packet_out_t *packet_out, xqc_usec_t now)
{
    ...
    if (packet_out->po_frame_types & (XQC_FRAME_BIT_STREAM | XQC_FRAME_BIT_DATAGRAM)) {
        send_ctl->ctl_app_bytes_send += packet_out->po_enc_size;
    }
...
xqc_send_ctl_on_ack_received(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_send_queue_t *send_queue, xqc_ack_info_t *const ack_info, xqc_usec_t ack_recv_time, xqc_bool_t ack_on_same_path)
{
    ...
    unsigned char has_acked = 0, update_largest_ack = 0, ignore_rtt = 0;
    ...
    if (update_largest_ack && has_ack_eliciting && ack_on_same_path) {
        if (!ignore_rtt) {
            ...
            xqc_log(conn->log, XQC_LOG_DEBUG, "|ignore_rtt:%ui|", ack_recv_time - send_ctl->ctl_largest_acked_sent_time[pns]);
        }
    ...
    if (xqc_send_ctl_is_ml_cc(send_ctl)) {
